...
class Nha_dat extends CI_Controller{
	...
	public function index($type){
		...
		if($type == 'nha_cho_thue')
		{
			$config['base_url'] = base_url('nha_dat/nha_cho_thue');
			$typeRent="Nhà cho thuê";
			$config['total_rows'] = $this->model_Realty->total_showListHostel($kind, $typeRent, $districtID, $provinceID, $size, $price);
		}
		elseif($type == 'nha_dat_can_ban')
		{
			$config['base_url'] = base_url('nha_dat/nha_dat_can_ban');
			$typeFindPartner = "Nhà đất cần bán";
	...
	/*public function nha_cho_thue1($provinceid, $districtid)
	{
		...
		$config['base_url'] = base_url('nha_dat/nha_cho_thue1')."/".$provinceid."/".$districtid;
