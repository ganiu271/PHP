...
class Nha_tro extends CI_Controller{
	...
	public function index($type){
		...
		if($type == 'nha_tro_cho_thue')
		{
			$config['base_url'] = base_url('nha_tro/nha_tro_cho_thue');
			$typeRent="Nhà trọ cho thuê";
			$config['total_rows'] = $this->model_Realty->total_showListHostel($kind, $typeRent, $districtID, $provinceID, $size, $price);
		}
		elseif($type == 'tim_nguoi_o_ghep')
		{
			$config['base_url'] = base_url('nha_tro/tim_nguoi_o_ghep');
			$typeFindPartner = "Tìm người ở ghép";
	...
	/*public function nha_tro_cho_thue1($provinceid, $districtid)
	{
		...
		$config['base_url'] = base_url('nha_tro/nha_tro_cho_thue1')."/".$provinceid."/".$districtid;
		...
		$data['isLogin'] = $isLogin;
		$data['menuactive'] = 'nhatro';
		$data['hinhthuc'] = 'nha_tro_cho_thue';
