...
class Can_ho extends CI_Controller{
	...
	public function register(&$data){
		...
		if($usernameRegister != NULL || $password1 != NULL || $password2 != NULL || $emailRegister != NULL || $isAgree != NULL)
		{
			...
			if(strlen($usernameRegister) < 5 || strlen($password1) < 5 || $password1 != $password2 || $isAgree != 'on'){
				...
				$password1 = $this::encryptIt($password1);
	...
	public function index($type){
		...
		if($type == 'can_ho_cho_thue')
		{
			$config['base_url'] = base_url('can_ho/can_ho_cho_thue');
			$typeRent="Căn hộ cho thuê";
			$config['total_rows'] = $this->model_Realty->total_showListHostel($kind, $typeRent, $districtID, $provinceID, $size, $price);
		}
		elseif($type == 'can_ho_can_ban')
		{
			$config['base_url'] = base_url('can_ho/can_ho_can_ban');
			$typeFindPartner = "Căn hộ cần bán";
	...
	/*public function can_ho_cho_thue1($provinceid, $districtid)
	{
		...
		$config['base_url'] = base_url('can_ho/can_ho_cho_thue1')."/".$provinceid."/".$districtid;
